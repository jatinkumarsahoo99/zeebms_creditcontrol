...
class ConnectorControl extends GetConnect {
    ...
    try {
      ...
      if (e.response?.statusCode == 401) {
        ...
        switch (e.type) {
          ...
          case DioErrorType.receiveTimeout:
          case DioErrorType.unknown:
            if (failed != null) failed!(failedMap);
            break;
          case DioErrorType.badResponse:
            if (failed != null) failed!(e.response?.data ?? "");
        }
      }
    }
  }
  ...
  POSTMETHOD_FORMDATAWITHTYPE(
    ...
    try {
      ...
      if (e.response?.statusCode == 401) {
        updateToken(() {
          POSTMETHOD_FORMDATAWITHTYPE(api: api, fun: fun, json: json);
        });
      } else if ([400, 403].contains(e.response?.statusCode)) {
        if (Get.isDialogOpen ?? false) {
          Get.back();
  ...
  POSTMETHOD_FORMDATA_HEADER(
    ...
    try {
      ...
      if (e.response?.statusCode == 401) {
        updateToken(() {
          POSTMETHOD_FORMDATA_HEADER(api: api, fun: fun, json: json);
        });
      } else if ([400, 403].contains(e.response?.statusCode)) {
        if (Get.isDialogOpen ?? false) {
          Get.back();
  ...
  GET_METHOD_CALL_HEADER(
    ...
    try {
      ...
      if (e.response?.statusCode == 401) {
        updateToken(() {
          GET_METHOD_CALL_HEADER(api: api, fun: fun, failed: failed);
        });
      } else if ([400, 403].contains(e.response?.statusCode)) {
        if (Get.isDialogOpen ?? false) {
          Get.back();
